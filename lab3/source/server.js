...
app.get('/', (req, res,next) => {
 ...
 if(req.query.imgFile){
  ...
  res.write("<table id='playlist_table'><tr><th>No.</td><th>URL</td><th>Type</td></tr></table><br></br>");

  res.write("<script>function addElement(el) {  var table = document.getElementById('playlist_table');  var row = table.insertRow();  var cell1 = row.insertCell(0);  var cell2 = row.insertCell(1);  var cell3 = row.insertCell(2);  cell1.innerHTML = table.rows.length-1;  cell2.innerHTML = el.getAttribute('src');if(el.getAttribute('id')=='videoAdd'){cell3.innerHTML = 'Video';}if(el.getAttribute('id')=='audioAdd'){cell3.innerHTML = 'Audio';}if(el.getAttribute('id')=='imgAdd'){ cell3.innerHTML = 'Image';}}</script>")
